...
분할 정복 접근법(MATRIX-MULTIPLY-RECURSIVE)은 n x n 행렬을 네 개의 n/2 x n/2 부분 행렬로 분할하여 8번의 재귀적 곱셈을 수행한다. 이 방식의 점화식은 T(n) = 8T(n/2) + Theta(n^2)이며, 해는 여전히 Theta(n^3)이다. 인덱스 계산을 통한 부분 행렬 분할은 Theta(1) 시간에 가능하지만, 4쌍의 부분 행렬 덧셈(결합 단계)이 Theta(n^2) 시간을 필요로 한다.
