...
<ArchitectureStack
  title="가상화 구조"
  topItems={[
    { label: "VM1", description: "Linux" },
    { label: "VM2", description: "Windows" },
    { label: "VM3", description: "Ubuntu" },
    { label: "VM4", description: "CentOS" }
  ]}
  baseLayer={{ label: "VMM (Hypervisor) + Hardware", highlight: true }}
/>
</div>

각 VM은 독립적인 OS를 실행하며 서로 격리된다.
...
VMM은 각 게스트에 **VCPU**(Virtual CPU)를 제공합니다. VCPU는 코드를 직접 실행하지 않으며, 게스트의 현재 CPU 상태를 표현하는 **데이터 구조**이다.
...
VMM은 OS와 유사한 기능(부팅 시 하드웨어 로드, 스케줄링, 메모리 관리)을 수행하지만, VMM의 "애플리케이션"은 게스트 OS이며 시스템 콜 대신 하드웨어 인터페이스를 제공한다는 점에서 구분된다.

## 예시

하나의 큰 건물을 여러 개의 독립적인 아파트로 나누는 것과 같다. 각 입주자는 자신의 공간이 독립적이라고 느끼지만, 실제로는 같은 건물의 자원(전기, 수도)을 공유한다.
...
- [하이퍼바이저 유형 (Type 0/1/2 Hypervisor)](/knowledge/os/hypervisor-types/) - 가상화를 구현하는 하이퍼바이저의 세 가지 유형
- [트랩 앤 에뮬레이트 (Trap-and-Emulate)](/knowledge/os/trap-and-emulate/) - 가상화의 기본 구현 기법인 트랩 기반 에뮬레이션
- [하드웨어 가상화 지원 (VT-x/AMD-V)](/knowledge/os/hw-virtualization/) - CPU가 가상화를 위한 전용 모드를 제공하는 하드웨어 기능
- [컨테이너 (Container)](/knowledge/os/container/) - 하드웨어 가상화 대신 OS 수준 격리를 제공하는 경량 가상화
- [VMM CPU 스케줄링 (VMM CPU Scheduling)](/knowledge/os/vmm-cpu-scheduling/) - VMM이 물리 CPU를 여러 VCPU에 시분할하는 방식
