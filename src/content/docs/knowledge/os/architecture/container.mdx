...
- [가상화 (Virtualization)](/knowledge/os/virtualization/) - 컨테이너와 비교되는 전체 하드웨어 가상화의 개념과 방식 분류
- [하이퍼바이저 유형 (Type 0/1/2 Hypervisor)](/knowledge/os/hypervisor-types/) - 컨테이너와 대비되는 VM 기반 가상화를 구현하는 하이퍼바이저
- [라이브 마이그레이션 (Live Migration)](/knowledge/os/live-migration/) - VM 환경에서의 서비스 무중단 이동 기술
- [JVM (Java Virtual Machine)](/knowledge/os/jvm/) - 프로그래밍 환경 수준의 가상화로, 컨테이너와는 다른 추상화 계층
