...
<ArchitectureStack
  title="하드웨어 가상화 모드 구조"
  topItems={[
    { label: "게스트 앱", description: "User mode" },
    { label: "게스트 앱", description: "User mode" }
  ]}
  baseLayer={{ label: "게스트 OS", description: "Kernel mode (Non-root)", highlight: true }}
/>

<StepFlow
  title="VM Exit / VM Entry 흐름"
  steps={[
    { label: "게스트 OS 실행", description: "Non-root mode" },
    { label: "특권 명령 실행", description: "VM Exit 발생" },
    { label: "VMM 처리", description: "Root mode" },
    { label: "VM Entry", description: "게스트로 복귀" }
  ]}
  cycle={true}
/>
...
<StepFlow
  title="EPT/RVI 주소 변환 과정"
  steps={[
    { label: "GVA", description: "Guest Virtual Address" },
    { label: "GPA", description: "Guest Physical Address" },
    { label: "HPA", description: "Host Physical Address" }
  ]}
/>
...
<StepFlow
  title="VT-d / IOMMU 동작"
  steps={[
    { label: "Protection Domain 설정", description: "디바이스별 접근 가능 메모리 영역 지정" },
    { label: "I/O 디바이스 할당", description: "특정 VM에 디바이스 전용 할당" },
    { label: "DMA 주소 자동 변환", description: "IOMMU가 GPA → HPA 변환" }
  ]}
/>
...
<LayerStack
  ...
  layers={[
    { label: "EL0", description: "사용자 애플리케이션" },
    { label: "EL1", description: "게스트 OS 커널" },
    { label: "EL2", description: "하이퍼바이저", highlight: true },
    { label: "EL3", description: "Secure Monitor" }
  ]}
/>
...
- [트랩 앤 에뮬레이트 (Trap-and-Emulate)](/knowledge/os/trap-and-emulate/) - 하드웨어 가상화 이전의 기본 가상화 기법
- [바이너리 변환 (Binary Translation)](/knowledge/os/binary-translation/) - 하드웨어 가상화가 대체한 소프트웨어 기반 x86 가상화 기법
- [가상화 (Virtualization)](/knowledge/os/virtualization/) - 가상화의 전체 개념과 다양한 방식 분류
- [하이퍼바이저 유형 (Type 0/1/2 Hypervisor)](/knowledge/os/hypervisor-types/) - 하드웨어 가상화를 활용하는 하이퍼바이저의 유형별 분류
- [VMM 메모리 관리 (VMM Memory Management)](/knowledge/os/vmm-memory-management/) - EPT/RVI와 연계되는 VMM의 메모리 관리 기법
