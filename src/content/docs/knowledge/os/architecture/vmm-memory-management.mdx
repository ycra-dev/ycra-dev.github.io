...
- [VMM CPU 스케줄링 (VMM CPU Scheduling)](/knowledge/os/vmm-cpu-scheduling/) - 메모리 관리와 함께 VMM의 핵심 자원 관리 기능인 CPU 스케줄링
- [하드웨어 가상화 지원 (VT-x/AMD-V)](/knowledge/os/hw-virtualization/) - EPT/RVI 등 메모리 가상화를 하드웨어로 지원하는 기능
- [가상화 (Virtualization)](/knowledge/os/virtualization/) - 메모리 오버커밋을 포함한 가상화의 전체 개념
- [바이너리 변환 (Binary Translation)](/knowledge/os/binary-translation/) - Nested Page Tables와 함께 사용되는 메모리 관리 기법
- [라이브 마이그레이션 (Live Migration)](/knowledge/os/live-migration/) - 메모리 페이지 전송이 핵심인 VM 이동 기술
