...
DFS 캐싱은 네트워크 트래픽과 디스크 I/O를 줄이기 위해 클라이언트에 데이터를 캐싱하고, 캐시된 복사본과 서버의 마스터 복사본 간 일관성을 유지하는 메커니즘입니다. DFS 캐싱은 네트워크 가상 메모리에 비유할 수 있습니다 (백킹 스토어가 원격 서버).
...
| 위치 | 장점 | 단점 |
|------|------|------|
| **디스크 캐시** | 신뢰성 (재부팅 후에도 유지) | 느린 접근 속도 |
| **메모리 캐시** | 빠른 접근, 디스크리스 지원 | 휘발성 (크래시 시 손실) |

현대 구현은 주로 메모리 캐시를 사용합니다.
...
NFS는 데이터에 주기적 flush, **메타데이터는 동기적 write** (구조 손상 방지)를 사용합니다.
...
```
[서버 주도 일관성 관리]
Server: Client A가 파일 F를 읽기 모드로 캐시 중
        Client B가 쓰기 모드로 열려고 함 → 충돌 감지

Option 1: Client A에 캐시 무효화 콜백 전송
Option 2: 파일 F에 대해 캐싱 비활성화
```
...
```
Write-on-Close 시나리오:
Client A: open(file.txt)      → 캐시에 복사
Client A: write("Hello")      → 로컬 캐시에만 쓰기
Client A: write(" World")     → 로컬 캐시에만 쓰기
Client A: close()             → 이 시점에 서버로 전송

-- 그 사이 Client B가 같은 파일을 열었다면? --
→ 일관성 문제 발생 가능
```
