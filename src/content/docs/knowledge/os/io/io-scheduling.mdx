...
- [HDD (Hard Disk Drive)](/knowledge/os/hdd/) - 기계식 탐색으로 인해 I/O 스케줄링이 가장 큰 효과를 보는 장치
- [SSD (Solid-State Drive)](/knowledge/os/ssd/) - 탐색 시간이 없어 NOOP 스케줄러가 적합한 저장장치
- [블로킹 vs 논블로킹 I/O](/knowledge/os/blocking-nonblocking-io/) - 블로킹 I/O 호출 시 요청이 대기 큐에 추가되어 스케줄링 대상이 됨
- [디바이스 드라이버](/knowledge/os/device-driver/) - 스케줄러가 재배치한 요청을 장치에 전달하는 커널 모듈
- [UNIX 블록 버퍼 캐시](/knowledge/os/unix-block-buffer-cache/) - 캐시 미스 시 디스크 I/O 요청이 발생하여 스케줄링 대상이 됨
