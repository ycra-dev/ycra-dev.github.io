...
CPU가 장치의 상태 레지스터를 반복적으로 읽어 장치가 준비되었는지 확인하는 I/O 방식이다. CPU와 장치 간의 속도 차이로 인한 동기화 문제를 해결하는 가장 단순한 방법으로, 추가 하드웨어 없이 소프트웨어만으로 구현할 수 있다.

## 동작 원리 (Handshaking Protocol)

장치 컨트롤러에 상태를 나타내는 busy 비트와 명령 준비를 나타내는 command-ready 비트가 있다고 가정한다:
...
프린터에 데이터를 보낼 때, CPU가 프린터 상태 레지스터를 계속 확인하면서 "준비됐어?"라고 묻고, 준비되면 다음 바이트를 전송한다.
