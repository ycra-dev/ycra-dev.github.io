...
**Blocking I/O**는 I/O 완료까지 호출 스레드가 대기 상태로 전환되는 방식이다.
**Non-blocking I/O**는 I/O 완료 여부와 관계없이 즉시 반환하여 스레드가 계속 실행되는 방식이다.

I/O 장치는 CPU보다 훨씬 느려서, 두 방식은 대기 시간을 어떻게 처리할지에 대한 서로 다른 접근법을 제공한다.
