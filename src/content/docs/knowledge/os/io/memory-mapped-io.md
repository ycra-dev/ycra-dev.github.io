...
장치 컨트롤러의 레지스터를 CPU의 메모리 주소 공간에 매핑하여, 일반 메모리 접근 명령어로 I/O를 수행하는 방식입니다. 별도의 I/O 명령어(IN, OUT) 없이 표준 데이터 전송 명령어(load/store)로 장치를 제어할 수 있습니다.
...
그래픽 메모리에 화면 데이터를 직접 쓰면, 그래픽 컨트롤러가 해당 내용을 읽어 모니터에 표시합니다. 수백만 바이트의 그래픽 데이터를 메모리 쓰기로 빠르게 전송할 수 있습니다.
