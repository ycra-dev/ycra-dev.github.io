...
- [UNIX 파일 디스크립터](/knowledge/os/unix-file-descriptor/) - VFS의 file 객체를 프로세스에서 참조하는 정수 핸들
- [UNIX 아이노드 (Inode)](/knowledge/os/unix-inode/) - VFS inode 객체의 원본인 디스크 상의 파일 메타데이터 구조
- [디바이스 드라이버](/knowledge/os/device-driver/) - VFS와 유사하게 표준 인터페이스로 장치를 추상화하는 커널 모듈
- [UNIX 블록 버퍼 캐시](/knowledge/os/unix-block-buffer-cache/) - VFS를 통한 파일 I/O가 경유하는 디스크 블록 캐시 계층
- [NTFS 구조](/knowledge/os/ntfs-structure/) - VFS 위에서 동작하는 Windows 파일 시스템의 내부 구조
