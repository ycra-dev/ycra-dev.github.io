...
- [VFS (Virtual File System)](/knowledge/os/vfs/) - 파일 시스템 종류를 추상화하는 계층으로, 드라이버와 유사한 표준 인터페이스 패턴 사용
- [Interrupt-Driven I/O](/knowledge/os/interrupt-driven-io/) - 드라이버가 I/O 완료 시 인터럽트를 받아 처리하는 방식
- [DMA (Direct Memory Access)](/knowledge/os/dma/) - 드라이버가 DMA 컨트롤러를 설정하여 대량 데이터를 전송하는 기법
- [I/O Manager와 IRP (Windows)](/knowledge/os/windows-io-manager/) - Windows에서 드라이버 스택을 통해 I/O를 처리하는 구조
- [인터럽트 (Interrupt)](/knowledge/os/interrupt/) - 장치가 드라이버에 이벤트를 알리는 하드웨어 신호 메커니즘
