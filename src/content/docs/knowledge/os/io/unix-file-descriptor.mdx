...
- [VFS (Virtual File System)](/knowledge/os/vfs/) - fd가 가리키는 file 객체를 관리하는 가상 파일 시스템 계층
- [UNIX 아이노드 (Inode)](/knowledge/os/unix-inode/) - fd → file → inode 참조 체인의 최종 대상인 파일 메타데이터 구조
- [블로킹 vs 논블로킹 I/O](/knowledge/os/blocking-nonblocking-io/) - fd에 대해 블로킹 또는 논블로킹 모드를 설정하여 I/O 동작 방식을 결정
- [UNIX 블록 버퍼 캐시](/knowledge/os/unix-block-buffer-cache/) - fd를 통한 read()/write()가 경유하는 커널 캐시 계층
- [시스템 콜 (System Call)](/knowledge/os/system-call/) - open(), read(), write(), close() 등 fd 기반 I/O를 수행하는 커널 인터페이스
