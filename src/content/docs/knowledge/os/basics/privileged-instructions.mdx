...
- [이중 모드 연산 (Dual-Mode Operation)](/knowledge/os/dual-mode-operation/) - 특권 명령어 실행을 위한 커널 모드와 사용자 모드 구분
- [트랩 (Trap)](/knowledge/os/trap/) - 사용자 모드에서 특권 명령어 실행 시도 시 발생하는 소프트웨어 인터럽트
- [타이머 (Timer)](/knowledge/os/timer/) - 타이머 수정이 특권 명령어로 분류되어 커널 모드에서만 가능
- [인터럽트 벡터 (Interrupt Vector)](/knowledge/os/interrupt-vector/) - 특권 명령어 위반 시 트랩이 전달되는 인터럽트 핸들러 주소 배열
- [시스템 콜 (System Call)](/knowledge/os/system-call/) - 사용자 프로그램이 특권 작업을 안전하게 요청하는 방법
