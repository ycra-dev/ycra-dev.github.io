...
<StepFlow
  steps={[
    { label: "User Mode", description: "사용자 프로세스 실행 (bit=1)" },
    { label: "System Call", description: "시스템 콜 호출" },
    { label: "Trap", description: "mode bit → 0 전환" },
    { label: "Kernel Mode", description: "커널에서 시스템 콜 실행" },
    { label: "Return", description: "mode bit → 1 전환" },
    { label: "Resume", description: "사용자 프로세스 실행 재개" }
  ]}
  cycle={true}
/>
...
- [특권 명령어 (Privileged Instructions)](/knowledge/os/privileged-instructions/) - 커널 모드에서만 실행 가능한 기계 명령어
- [트랩 (Trap)](/knowledge/os/trap/) - 사용자 모드에서 커널 모드로 전환을 유발하는 소프트웨어 인터럽트
- [시스템 콜 (System Call)](/knowledge/os/system-call/) - 사용자 모드에서 커널 서비스에 안전하게 접근하는 방법
- [타이머 (Timer)](/knowledge/os/timer/) - 운영체제가 CPU 제어를 유지하도록 보장하는 보호 장치
- [커널 (Kernel)](/knowledge/os/kernel/) - 커널 모드에서 실행되는 운영체제의 핵심 프로그램
