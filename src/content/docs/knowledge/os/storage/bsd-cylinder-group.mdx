...
- [UNIX 아이노드 (Inode)](/knowledge/os/unix-inode/) - 실린더 그룹 내에 배치되는 파일 메타데이터 구조
- [HDD (Hard Disk Drive)](/knowledge/os/hdd/) - 실린더 그룹이 최적화하는 대상인 기계식 디스크의 물리 구조
- [I/O 스케줄링](/knowledge/os/io-scheduling/) - 실린더 그룹과 함께 디스크 seek 시간을 줄이는 기법
- [저널링 파일 시스템](/knowledge/os/journaling/) - FFS 이후 등장한 크래시 복구 기법으로, 슈퍼블록 복제와 다른 접근
- [논리 블록 주소 (LBA)](/knowledge/os/lba/) - 물리적 실린더 위치를 추상화한 논리 주소 체계
