...
│          물리 디스크들                   │
├────────────┬────────────┬───────────────┤
│  Volume 1  │  Volume 2  │   Volume 3    │  ← 고정 크기
├────────────┼────────────┼───────────────┤
│    FS1     │    FS2     │     FS3       │  ← 볼륨에 종속
└────────────┴────────────┴───────────────┘
  FS1 가득 차도 FS3 공간 사용 불가
...
- **스냅샷**: 특정 시점 상태를 즉시 보존
- **쿼터/예약**: 파일 시스템별 최대/최소 용량 제어
- **압축**: 투명한 데이터 압축 (LZ4, ZSTD 등)
- **중복 제거**: 동일 데이터 블록 1회만 저장
...
- [RAID 레벨 (RAID Levels)](/knowledge/os/raid-levels/) - 전통적 RAID와 RAID-Z 비교
- [오류 검출과 정정 (ECC)](/knowledge/os/ecc/) - 데이터 무결성 기술
- [저널링 파일 시스템 (Journaling)](/knowledge/os/journaling/) - 전통적 일관성 보장 기법과 대비
- [저장장치 초기화](/knowledge/os/storage-initialization/) - 포매팅, 파티셔닝, 볼륨
