...
- [NTFS 구조](/knowledge/os/ntfs-structure/) - $LogFile이 저장되는 NTFS 볼륨의 MFT 기반 구조
- [저널링 파일 시스템](/knowledge/os/journaling/) - UNIX 계열 파일 시스템의 WAL 기반 저널링 기법으로, NTFS 복구와 유사한 접근
- [ZFS (Zettabyte File System)](/knowledge/os/zfs/) - COW 방식으로 저널링 없이도 일관성을 보장하는 대안적 접근
- [I/O Manager와 IRP (Windows)](/knowledge/os/windows-io-manager/) - NTFS 드라이버가 IRP를 처리하며 $LogFile에 트랜잭션을 기록
- [UNIX 블록 버퍼 캐시](/knowledge/os/unix-block-buffer-cache/) - NTFS 복구가 해결하는 dirty 캐시 손실 문제의 UNIX 대응물
