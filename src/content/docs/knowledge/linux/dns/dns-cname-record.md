...
updated: 2026-02-17
draft: true
slug: knowledge/linux/dns-cname-record
sidebar:
  order: 11
---
...
**존 정점(Apex)에서의 CNAME 금지**: RFC1033에 따라 존의 정점(예: example.com)에는 CNAME을 사용할 수 없고 반드시 A/AAAA 레코드를 사용해야 한다. 클라우드 환경에서 IP가 변경되는 경우, AWS Route 53이나 CloudFlare 같은 관리형 DNS가 CNAME처럼 설정하되 실제로는 A 레코드를 서비스하는 우회 기능을 제공한다.
...
- [dns-resource-record](/knowledge/linux/dns-resource-record/) - CNAME을 포함하는 레코드 체계
- [dns-a-record](/knowledge/linux/dns-a-record/) - CNAME이 궁극적으로 가리키는 레코드
- [dns-zone](/knowledge/linux/dns-zone/) - CNAME이 속하는 존
- [dns-ptr-record](/knowledge/linux/dns-ptr-record/) - CNAME이 아닌 실제 이름을 참조해야 하는 레코드
