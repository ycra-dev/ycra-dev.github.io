...
<Mermaid diagram={`
graph TD
    kubectl["kubectl create deployment web-server --image=nginx:1.25"] --> API["API Server (검증)"]
    API --> etcd["etcd (객체 데이터 영구 저장)"]
    API --> Controller["컨트롤러"]
    API --> Scheduler["스케줄러"]
`} />
...
- [래프트 합의 (Raft Consensus)](/knowledge/distributed-systems/raft-consensus/) - etcd가 데이터 일관성을 위해 사용하는 분산 합의 알고리즘
