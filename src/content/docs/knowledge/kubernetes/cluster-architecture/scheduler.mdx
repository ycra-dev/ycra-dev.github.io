...
<Mermaid diagram={`
graph TD
    D["Deployment 생성"] --> C["Controller가 Pod 객체 생성"]
    C --> S["Scheduler가 미할당 Pod 감지"]
    S --> E["각 노드의 가용 리소스 평가"]
    E --> N1["Worker Node 1: CPU 80% ❌"]
    E --> N2["Worker Node 2: CPU 30% ✅"]
    E --> N3["Worker Node 3: CPU 50% ✅"]
    N2 --> A["Worker Node 2를 선택하여 Pod에 할당"]
    A --> K["Worker Node 2의 Kubelet이 Pod 실행"]
`} />
...
NAME                          ...  IP          NODE
web-server-9d785b578-58vhc    ...  10.244.1.5  worker1
web-server-9d785b578-jmnj8    ...  10.244.2.4  worker2
```
